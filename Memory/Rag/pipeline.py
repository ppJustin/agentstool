...
import time
import json
from Memory.memory.embedding import get_text_embedder, get_dimension
from Memory.memory.storage.qdrant_store import QdrantVectorStore


def _get_markitdown_instance():
    """
    用于将各种格式转换为Markdown文本
    """
    try:
...
def build_graph_from_chunks(neo4j, chunks: List[Dict]) -> None:
    '''
    为每个文档创建Document节点,为每个块创建Memory节点
    '''
    created_docs = set()
    for ch in chunks:
        mem_id = ch["id"]
...
def _create_default_vector_store(dimension: Optional[int] = None) -> QdrantVectorStore:
    ...
    # 使用连接管理器
    from Memory.memory.storage.qdrant_store import QdrantConnectionManager
...
def _prompt_mqe(query: str, n: int) -> List[str]:
    '''
    MQE（Multi-Query Expansion）查询扩展
    '''
    try:
...
def _try_load_cross_encoder(model_name: str = "cross-encoder/ms-marco-MiniLM-L-6-v2"):
    #使用交叉编码器：双输入单输出的模型，同时处理查询和文档，输出一个相似度分数，主要用于精排文档，加载很慢，普通双编码器用于初筛
    try:
...
def compute_graph_signals_from_pool(vector_hits: List[Dict], same_doc_weight: float = 1.0, proximity_weight: float = 1.0, proximity_window_chars: int = 1600) -> Dict[str, float]:
    """
    计算图结构信号,把块按照所属文档分组，并计算文档密度得分，加上邻近度（对每个块计算和临近块的距离）得分
    先找到块最多的文档，然后再找到块分数最高的块，重新排列输出
    """
...
def expand_neighbors_from_pool(selected: List[Dict], pool: List[Dict], neighbors: int = 1, max_additions: int = 5) -> List[Dict]:
    '''
    从原始候选池中，找到选中片段在同一文档中的前/后邻居，扩展上下文
    '''
