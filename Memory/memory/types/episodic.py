...
class EpisodicMemory(BaseMemory):
    ...
    def __init__(self, config: MemoryConfig, storage_backend=None):
        ...
        self.sessions: Dict[str, List[str]] = {}  # session_id -> episode_ids   聊天记录索引
        
        # 模式识别缓存
        self.patterns_cache = {}        #寻找到的规律
    ...
    def add(self, memory_item: MemoryItem) -> str:
        ...
        self.sessions[session_id].append(episode.episode_id)    #构建索引列表
    ...
    def retrieve(self, query: str, limit: int = 5, **kwargs) -> List[MemoryItem]:
        ...
        if time_range is not None or importance_threshold is not None:
            ...
            candidate_ids = {d["memory_id"] for d in docs}      #候选id
        ...
        results: List[Tuple[float, MemoryItem]] = []        #分数/记忆
        seen = set()
        for hit in hits:
            ...
            # 检查是否已遗忘
            episode = next((e for e in self.episodes if e.episode_id == mem_id), None)  #找到一个就停止寻找
    ...
    def remove(self, memory_id: str) -> bool:
        ...
        for i, episode in enumerate(self.episodes):
            if episode.episode_id == memory_id:
                ...
                if session_id in self.sessions:     #session中存储的是一段对话过程
