...
class Perception:
    ...
    def __init__(
        self,
        perception_id: str,     #感知记忆id
        data: Any,              #感知记忆原始数据
        modality: str,          #感知记忆模态类型
        encoding: Optional[List[float]] = None,     #特征向量/嵌入向量
        ...
        self.data_hash = self._calculate_hash() #数据哈希值，唯一的标识符，便于比较不同的数据
    
    def _calculate_hash(self) -> str:
        """计算数据哈希"""
        if isinstance(self.data, str):  #文本类型
            return hashlib.md5(self.data.encode()).hexdigest()
        elif isinstance(self.data, bytes):  #二进制
...
class PerceptualMemory(BaseMemory):
    ...
    def __init__(self, config: MemoryConfig, storage_backend=None):
        ...
        # 感知数据存储（内存缓存）
        self.perceptions: Dict[str, Perception] = {}        #感知记忆实体（快照）
        self.perceptual_memories: List[MemoryItem] = []     #记忆项
        ...
        try:
            from transformers import CLIPModel, CLIPProcessor       #同时处理图像和文本的模型
    ...
    class _no_grad:     #上下文管理器，临时禁用PyTorch的梯度计算，节省内存和计算资源。
        def __enter__(self):
            try:
                import torch
