...
class WorkingMemory(BaseMemory):
    ...
    def __init__(self, config: MemoryConfig, storage_backend=None):
        ...
        # 纯内存TTL存活时间（分钟），可通过在 MemoryConfig 上配置存活时间来更改
        ...
        # 存储所有记忆条目的列表：工作记忆无需持久化存储
        self.memories: List[MemoryItem] = []
        
        # 使用优先级队列管理记忆 数字/时间戳/实际记忆对象
    ...
    def retrieve(self, query: str, limit: int = 5, user_id: Optional[str] = None, **kwargs) -> List[MemoryItem]:
        ...
        try:
            ...
            similarities = cosine_similarity(query_vector, doc_vectors).flatten()   #通过计算余弦相似度来计算记忆相关性
            
            # 存储向量分数
            for i, memory in enumerate(filtered_memories):
                vector_scores[memory.id] = similarities[i]      #记忆id-相似度
        ...
        for memory in filtered_memories:
            ...
            if query_lower in content_lower:
                ...
                intersection = query_words.intersection(content_words)  #计算交集
                if intersection:
                    keyword_score = len(intersection) / len(query_words.union(content_words)) * 0.8     #计算并集
        ...
        # 按分数排序并返回
        scored_memories.sort(key=lambda x: x[0], reverse=True)      #每个元素的第一个值，降序排列
    ...
    def _calculate_time_decay(self, timestamp: datetime) -> float:
        """计算时间衰减因子"""
        time_diff = datetime.now() - timestamp      #datetime对象支持加减法
        hours_passed = time_diff.total_seconds() / 3600     #将时间差转化为标准秒格式，再转化为小时
        
        # 指数衰减（工作记忆衰减更快）
        decay_factor = self.config.decay_factor ** (hours_passed / 6)  # 每6小时为一个衰减周期
        return max(0.1, decay_factor)  # 最小保持10%的衰减权重
    ...
    def _expire_old_memories(self):
        """按TTL(存活时间)清理过期记忆，并同步更新堆与token计数"""
        #若没有记忆项
        if not self.memories:
            return
        cutoff_time = datetime.now() - timedelta(minutes=self.max_age_minutes)  #timedelta:Python中的时间间隔的对象
        #截断过期时间，过滤保留的记忆
        kept: List[MemoryItem] = []
        removed_token_sum = 0   #移除的总词元数
        ...
        for mem in self.memories:
            ...
            heapq.heappush(self.memory_heap, (-priority, mem.timestamp, mem))       #取负值实现最大堆，正常情况下堆顶是最小值的元素
    ...
    def _update_heap_priority(self):
        """更新堆中记忆的优先级"""
        # 简单实现：重建堆
        self.memory_heap = []
        for mem in self.memories:
